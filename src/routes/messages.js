    ...
    try {
      ...
      const result = await query(
        `SELECT
           ...
           CASE
             WHEN pc.user_initiator = $1 THEN CONCAT(u2.firstname, ' ', u2.surname)
             ELSE CONCAT(u1.firstname, ' ', u1.surname)
    ...
    try {
      ...
      const result = await query(
        `SELECT m.id, m.sender_id, m.conversation_id, m.content, m.message_type,
                ...
                CONCAT(u.firstname, ' ', u.surname) AS sender_name, u.user_name AS sender_username,
