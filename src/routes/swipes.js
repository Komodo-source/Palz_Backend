...
async function swipeRoutes(app) {
  ...
  app.get('/matches', { preHandler: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      const result = await query(
        `SELECT
           u.id, CONCAT(u.first_name, ' ', u.surname) AS full_name, u.user_name, u.profile_image, u.bio,
  ...
  app.get('/likes', { preHandler: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      const result = await query(
        `SELECT u.id, CONCAT(u.first_name, ' ', u.surname) AS full_name, u.user_name, u.profile_image, u.bio,
